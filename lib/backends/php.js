...
/** ThinkPHP 典型目录标记（TP6+ app，TP5 application） */
const PHP_BACKEND_MARKERS = ['app', 'application', 'config', 'route']
...
function validatePhpBackend (sourceDir) {
  ...
  const hasApplication = fs.existsSync(path.join(sourceDir, 'application'))
  ...
  if (hasApplication && (hasConfig || hasThink || fs.existsSync(path.join(sourceDir, 'public')))) {
    hints.push('检测到 application/ 目录，按 ThinkPHP 5 结构处理')
    return { valid: true, framework: 'thinkphp', hints }
  }

  if (hasApp) {
    hints.push('检测到 app/ 目录，按 ThinkPHP 6+ 结构处理')
    return { valid: true, framework: 'thinkphp', hints }
  }
  ...
  if (hasComposer) {
    hints.push('未检测到 app/ 或 application/，但存在 composer.json，按通用 PHP 项目处理')
  ...
  hints.push('若确为 PHP 后端，请确认 --source 指向含 app/ 或 application/ 的项目根（如 trunk）')
